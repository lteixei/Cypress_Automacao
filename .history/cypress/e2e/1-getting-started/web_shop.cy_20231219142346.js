...
cy.get(':nth-child(7) > .product-item > .picture > a > img')
...
cy.iframe ('aut-iframe').find ('Continue').should (' be.visible ').click()
  //cy.frameLoaded('[title="Your project: 'Test Project'"]')
  // after the frame has loaded, we can use "cy.iframe()"
  // to retrieve it
  cy.iframe().find('#shipping-buttons-container > .button-1').should('Continuar').click()
//cy.get('#billing-buttons-container > .button-1').click()
