...
describe('DEMO WEB SHOP', () => {
    ...
    //LOGIN 
        ...
        ////
        cy.wait(35000) // wait for 2 seconds
        cy.get('.opcao__email').click();
        cy.get('.email__dialog > .main__dialog > .buttons > .btn__primary').click();
        //
        DATA
cy.get('#date7 > .row__dia > .radio__label > .checkmark')
BOTAO
cy.get('.box__voltar__mobile > .btn')
NÃO QUERO ATIVAR
cy.get('.row__buttons > .btn__outline__primary')

cy.get(':nth-child(2) > .label__checkbox')
cy.get(':nth-child(2) > .label__checkbox')
cy.get('.content__termos > .btn')
