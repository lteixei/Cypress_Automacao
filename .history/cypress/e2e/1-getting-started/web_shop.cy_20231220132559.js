...
Cypress._.times(5, () => {
  ...
  if ($body.find('cy.get(':nth-child(1) > .remove-from-cart > input'):checked').lenght) {
  cy.log('check box was checked')
  return
  }
  cy.log('check box was unchecked')
  $body.find('cy.get(':nth-child(1) > .remove-from-cart > input')').click()
  return
  })
  cy.get('cy.get(':nth-child(1) > .remove-from-cart > input')')
  .should('be.checked')
  cy.get('.update-cart-button').click()
  })
  })
